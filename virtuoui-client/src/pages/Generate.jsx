...
import {
    FiZap, FiSave, FiEye, FiCode, FiArrowRight,
...
function Generate () {
    ...
    const handleGenerate = async() => {
        if(!prompt.trim() || lowCredits) {
            return;
        }
        setGenerated(null);
        setGenerating(true);
        try{
            const {data} = await axios.post(ServerUrl+"/api/v1/component/generate",
                {prompt},
    ...
    return (
        ...
        >
            ...
            <div className="relative z-20 max-w-5xl mx-auto px-4 py-12">
                <motion.div
                ...
                >
                    ...
                    style={{fontFamily: "'Space Grotesk', sans-serif", letterSpacing: "-0.03rem"}}>
...

                {
                    userRole === "user" && (
                        ...
                        className="flex justify-end mb-4">
                            ...
                            style={{
                                background: lowCredits ? "rgba(239,68,68,0.1)" : "rgba(99,102,241,0.25)"
                            }}>
...

                {
                    lowCredits && (
                        ...
                        style={{background:"rgba(239,68,68,0.08)", border:"1px solid regba(239,68,68,0.2)"}}
                        >
                            <FiAlertCircle size={16} className="text-red-400 shrink-0"/>
                            <p className="text-sm text-red-300">
                                You need at least <span className="font-bold text-red-400">50 credits
                                </span> to generate component.
                            </p>
                            <button 
                            onClick={()=>navigate("/pricing")}
                            className="ml-auto flex items-center gap-1.5 px-3 py-1.5 rounded-lg
                            text-xs font-semibold transition-all cursor-pointer border-none whitespace-nowrap"
                ...
                >
                    <div className="flex items-start gap-3 p-4">
                        <FiZap size={20} className="text-indigo-400 mt shrink-0"/>
                        <textarea 
                            ...
                            <div className="flex items-center gap-3 px-5 pb-5 pt-1 flex-wrap">
                                {userRole === "admin" && (
                                    ...
                                    style={{
                                        ...
                                        border: savedComponentId ? "1px solid rbga(16,185,129,0.3)" : "rgba(255,255,255,0.1)",
                                        color: savedComponentId ? "#34d399" : "#fff"
                                    }}>
                                        {saving ? <motion.span animate={{rotate:360}}
                                        transition={{repeat:Infinity, duration:1, ease:"linear"}}>
                                            <FiLoader size={14}/></motion.span> 
...

                                    {
                                        savedComponentId && !published && (
                                            ...
                                            text-sm font-semibold transition-all disabled:opacity-40 cursor-pointer"
                                            style={{
                                    ...
                                    {published && (
                                        ...
                                        style={{
                                            ...
                                            border:  "1px solid rbga(16,185,129,0.3)",
                                            color: "#34d399"
                                        }}>
